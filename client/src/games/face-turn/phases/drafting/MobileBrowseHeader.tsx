...
function FilterChips<T extends string>({
	...
	return (
		<div className="flex gap-1.5 overflow-x-auto scrollbar-none min-w-0 -mx-4 px-4">
			{options.map((opt) => {
				const isAll = opt === "all";
				const theme = !isAll
				...
				return (
					<button
						...
						className={cn(
							"shrink-0 px-3 py-1.5 rounded-full text-[11px] font-black uppercase tracking-widest border transition-all cursor-pointer active:scale-95",
							isActive
								? "text-white"
	...
	return (
		<div ref={containerRef} className="relative shrink-0">
			<button
				...
				onClick={() => setOpen((v) => !v)}
				className={cn(
					"flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] font-bold uppercase tracking-widest transition-all cursor-pointer active:scale-95",
					activeCount > 0 || open
			...
			{open && (
				<div className="absolute z-20 top-full right-0 mt-1 w-72 rounded-lg border border-white/15 ft-panel-ink shadow-xl overflow-hidden">
...
export function MobileBrowseHeader({
	...
	crewFilter: "all" | CrewClass;
	setCrewFilter: (filter: "all" | CrewClass) => void;
	moveFilter: "all" | MoveType;
	setMoveFilter: (filter: "all" | MoveType) => void;
	pickedFilter: PickedFilterValue;
	...
	return (
		<div className="flex flex-col gap-2 px-4 pt-2.5 pb-2 border-b border-white/10">
			...
			{tab === "crew" && (
				<FilterChips
					options={
						["all", "striker", "defender", "collector", "hider"] as const
					}
					active={crewFilter}
			...
			{tab === "moves" && (
				<FilterChips
					options={["all", "burst", "slow", "active"] as const}
					active={moveFilter}
